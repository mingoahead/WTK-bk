...
/*! Structure clickedPoint to store the x,y,z and intensity value of a
* point in the image
*/
class ClickPoint
{
...
template <class imType> class SliceView
{
    enum { Dimension = 3 };

public:
    // some typedefs
    typedef Image<imType, Dimension>           ImageType;
...
protected:
    ...
    unsigned long            cDimSize[Dimension];
    double                   cOrigin[Dimension];
    double                   cSpacing[Dimension];
    ...
    double        cClickSelect[Dimension];
    double        cClickSelectV;
    void          (* cClickSelectCallBack)(double x, double y, double z, double v);
    void          * cClickSelectArg;
    void          (* cClickSelectArgCallBack)(double x, double y, double z, double v, void *clickSelectArg);

    double       cBoxMin[Dimension];
    double       cBoxMax[Dimension];
    void         (* cClickBoxCallBack)(double minX, double minY, double minZ, double maxX, double maxY, double maxZ);
    void         * cClickBoxArg;
    void         (* cClickBoxArgCallBack)(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, void * clickBoxArg);

    /* max, min image values */
    double       cIWMin;
    double       cIWMax;
    ...
    bool        cFlipX[Dimension];
    bool        cFlipY[Dimension];
    bool        cFlipZ[Dimension];
    bool        cTranspose[Dimension];

    /* 缩放系数，初始值为 1.0 */
    double              cWinZoom;
    unsigned int        cWinOrder[Dimension];
    ...
    int         cWinCenter[Dimension];
    void        (* cWinCenterCallBack)(void);
    void        * cWinCenterArg;
    void        (* cWinCenterArgCallBack)(void * winCenterArg);

    bool        cViewAxisLabel;
    char        cAxisLabelX[Dimension][80];
    char        cAxisLabelY[Dimension][80];
...
public:
    ...
    *  being viewed */
    ...
    virtual void clickSelect(double newX, double newY, double newZ);
    double        clickSelectX(void);
    double        clickSelectY(void);
    double        clickSelectZ(void);
    void         clickSelectCallBack( void (*newClickSelectCallBack)(double, double, double, double) );
    void         clickSelectCallBack( void (*newClickSelectArgCallBack)(double, double, double, double, void *), void * newClickSelectArg);
    ...
    * i = 0, to get the 2nd last point i = 1, and so on
    ...
    inline int numClickedPointsStored() { return cClickedPoints.size(); }

    void         maxClickedPointsStored(unsigned int i);
    unsigned int maxClickedPointsStored();

    inline void clearClickedPointsStored() { cClickedPoints.clear(); }
    inline void deleteLastClickedPointsStored() { cClickedPoints.pop_front(); }
    ...
    void            boxMin(double minX, double minY, double minZ);
    virtual void    boxMax(double maxX, double maxY, double maxZ);
    void            clickBoxCallBack(
        void (*newClickBoxCallBack)(double, double, double,
                                    double, double, double));
    void            clickBoxCallBack(
        void (*newClickBoxArgCallBack)(double, double, double,
                                       double, double, double,
                                       void *),
        void * newClickBoxArg);

    void        iwMin(double newIWMin);
    double       iwMin(void);
    void        iwMax(double newIWMax);
    double       iwMax(void);
    ...
    void viewCrosshairs(bool crosshairs) {
    ...
    void SetFastMoveValue(int moval) {
        fastMovVal = moval;    //fast moving pace
    }
    int GetFastMoveValue() {
        return fastMovVal;
    }
    //how many single step moves before fast moving
    void SetFastMoveThresh(int thresh) {
        fastMovThresh = thresh;
    }
    int GetFastMoveThresh() {
        return fastMovThresh;
    }
};



template<class imType> SliceView<imType>::SliceView(int x, int y, int w, int h, const char * )
{
    cX = x;
    cY = y;
    cW = w;
...
//
template <class imType>
const typename SliceView<imType>::ImageConstPointer &SliceView<imType>::GetInputImage(void) const
{
    return cImData;
}
...
template <class imType>
typename SliceView<imType>::ImagePointer SliceView<imType>::GetInputImage(void)
{
...
clickSelectCallBack(void
                    (*newClickSelectCallBack)(double _x, double _y,
                            double _z, double _v))
{
...
void SliceView<imType>::
clickSelectCallBack(void
                    (*newClickSelectArgCallBack)(double _x, double _y,
                            double _z, double _v,
                            void *clickSelectArg),
...
//
template <class imType>
void SliceView<imType>::clickSelect(double newX, double newY, double newZ)
{
    cClickSelect[0] = newX;
    if(cClickSelect[0]<0)
        cClickSelect[0] = 0;
...
template <class imType>
double SliceView<imType>::clickSelectX(void)
{
...
template <class imType>
double SliceView<imType>::clickSelectY(void)
{
...
template <class imType>
double SliceView<imType>::clickSelectZ(void)
{
...
template <class imType>
void SliceView<imType>::
clickBoxCallBack(void (*newClickBoxCallBack)(double minX, double minY,
...
template <class imType>
void SliceView<imType>::
clickBoxCallBack(void (*newClickBoxArgCallBack)(double minX, double minY,
                 double minZ, double maxX,
                 double maxY, double maxZ,
                 void * clickBoxArg),
                 void * newClickBoxArg)
...
template <class imType>
void SliceView<imType>::boxMin(double minX, double minY, double minZ)
{
    cBoxMin[0] = minX;
    cBoxMin[1] = minY;
    cBoxMin[2] = minZ;
...
template <class imType>
void SliceView<imType>::boxMax(double x, double y, double z)
{
    double x0, y0, z0;
    double x1, y1, z1;
...
//
template <class imType>
void SliceView<imType>::iwMin(double newIWMin)
{
    cIWMin = newIWMin;
    if(cIWCallBack != NULL)
        cIWCallBack();
...
template <class imType>
double SliceView<imType>::iwMin(void)
{
    return cIWMin;
}
...
//
template <class imType>
void SliceView<imType>::iwMax(double newIWMax)
{
    cIWMax = newIWMax;
    if(cIWCallBack != NULL)
        cIWCallBack();
...
template <class imType>
double SliceView<imType>::iwMax(void)
{
    return cIWMax;
}
...
//
template <class imType>
void SliceView<imType>::winZoom(double newWinZoom)
{
...
template <class imType>
double SliceView<imType>::winZoom(void)
{
    return cWinZoom;
}
